...
# This script first was copied from:
...
# But has been modified for OpenSUSE, not verified to work yet
...
if [ ! -e /etc/xrdp/startsuse-icewm.sh ]; then
cat >> /etc/xrdp/startsuse-icewm.sh << EOF
#!/bin/sh
export XDG_CURRENT_DESKTOP=ICEWM
exec /etc/xrdp/startwm.sh
EOF
chmod a+x /etc/xrdp/startsuse-icewm.sh
fi

# use the script to setup the ubuntu session
sed -i_orig -e 's/startwm/startsuse-icewm/g' /etc/xrdp/sesman.ini
